...
// 这里其实包含了两个不同的上下文，一个是函数执行上下文，一个是全局上下文
```
...
- `await` 后面的代码会被包装成微任务，在**微任务队列清空后执行**。
...
问：`await` 后面的代码是怎么执行的？\
答：`await` 会将后续代码包装成 `.then()` 回调，放入微任务队列，等到 Promise settled 执行。其实await包装的代码可能会更快，但是它是**严格排到当前执行上下文微任务的队尾**
